...
import { deleteDivision, getAllDivision } from '../../../API/DivisionApi';
...
const DivListing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAllDivision();
        setDivision(result);
  ...
  useEffect(() => {
    const mySearch = division.filter((item) =>
      item.Title
  ...
  const handleDelete = async (row) => {
    try {
      ...
      setDivision((prevDivision) =>
        prevDivision.filter((item) => item.Id !== row.Id),
      );
  ...
  return (
    <div>
      <Breadcrumb pageName="Division Listing" />
