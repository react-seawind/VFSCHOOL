...
import { deleteHolidayHW, getAllHolidayHW } from '../../API/HolidayHomeWorkApi';

const HolidayListing = () => {
  const [holidayhw, setholidayhw] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAllHolidayHW();
        setholidayhw(result);
  ...
  useEffect(() => {
    const mySearch = holidayhw.filter((item) =>
      item.Title
  ...
  const handleDelete = async (row) => {
    try {
      await deleteHolidayHW(row.Id);
      setholidayhw((prevholidayhw) =>
        prevholidayhw.filter((item) => item.Id !== row.Id),
      );
      setFilterData((prevFilterData) =>
        prevFilterData.filter((item) => item.Id !== row.Id),
      );
    } catch (error) {
      console.error('Error deleting holidayhw:', error);
    }
  };
    ...
    return (
      <div>
        <Button
          ...
          onClick={() => {
            navigate(`/holiday/edit/${rowData.Id}`);
          }}
        />
        <Button
          icon={<FaTrash />}
  ...
  return (
    <div>
      <Breadcrumb pageName="Holiday Homework Listing" />
