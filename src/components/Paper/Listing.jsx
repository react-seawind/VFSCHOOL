...
import { deleteExamPaper, getAllExamPaper } from '../../API/ExampaperApi';
...
const PaperListing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAllExamPaper();
        setexampaper(result);
  ...
  const handleDelete = async (row) => {
    try {
      ...
      setexampaper((prevexampaper) =>
        prevexampaper.filter((item) => item.Id !== row.Id),
      );
  ...
  useEffect(() => {
    const mySearch = exampaper.filter(
      (item) =>
  ...
  return (
    <div>
      <Breadcrumb pageName="Paper Listing" />
