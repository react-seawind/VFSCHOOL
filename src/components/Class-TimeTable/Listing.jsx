...
import { deleteClassTT, getAllClassTT } from '../../API/ClasstimetableApi';
import { format } from 'date-fns';

const ClassTimetableListing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAllClassTT();
        setclasstt(result);
          ...
          {selectedRow && selectedRow.Id === row.Id && (
            <div className="action-buttons  absolute z-99">
              <button
                ...
                onClick={() => {
                  setSelectedRow(null);
                  Navigate(`/classtimetable/edit/${row.Id}`);
                }}
              >
                Edit
              </button>
  ...
  return (
    <div>
      <Breadcrumb pageName="Class Timetable Listing" />
...
export default ClassTimetableListing;
